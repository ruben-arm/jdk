...
private:
  ...
  NMethodHeader::DeoptimizationStatus deoptimization_status() const {
    return Atomic::load(&_hdr->_deoptimization_status);
  }
  ...
  // For method handle intrinsics: Try MethodSimpleNonProfiled, MethodOptNonProfiled,
...
public:
  ...
  // header_begin() and header_end() would still point to the start of the nmethod object
  ...
  address deopt_handler_begin   () const { return           header_begin() + _hdr->_deopt_handler_offset    ; }
  address deopt_mh_handler_begin() const { return           header_begin() + _hdr->_deopt_mh_handler_offset ; }
...
#if INCLUDE_JVMCI
  Metadata** metadata_end       () const { return (Metadata**) (mutable_data_end() - _hdr->_jvmci_data_size); }
  address jvmci_data_begin      () const { return               mutable_data_end() - _hdr->_jvmci_data_size; }
  ...
  bool update_recompile_counts() const {
    ...
    // or the nmethod is not marked for deoptimization at all (nmethod::header::not_marked).
  ...
  bool  has_method_handle_invokes() const         { return _hdr->_has_method_handle_invokes; }
  void  set_has_method_handle_invokes(bool z)     { _hdr->_has_method_handle_invokes = z; }
...
public:
 ...
 public:
  ...
  // Now that the _osr_entry_point and _state variables live in the C heap, they can be directly
  // accessed only by the _hdr pointer. hdr_offset() returns the offset of the _hdr pointer from the
  // nmethod which can then be used to compute offsets for osr_entry_point and _state
